...
from research import APIResearch, OpenLibraryAPIResearch
...
def main():
    ...
    def ajouter_article():
        ...
        # Vérifier si l'année de publication est un entier valide
        prix: float = get_price()
        

        # Créer la BD
        nouvel_article = BD(title, auteur_name, isbn, int(first_publish_year), language, code_barre, prix)
    ...
    def modifier_article():
        ...
        while True:
            valeur_modification = input(f"Entrez la nouvelle valeur de {choix_variable}: ")

            if choix_variable == "first_publish_year":
                if not valeur_modification.isdigit():
                    print("\u26A0️ L'année de publication et le prix doivent être des nombres entiers. \u26A0️")
                    continue
                break# Recommencer la boucle pour demander à nouveau la saisie
            if choix_variable == "prix":
                try:
                    valeur_modification = float(valeur_modification)  # Tente de convertir en flottant
                    break
                except ValueError:
    ...
    def rechercher_openlibrary():
        isbn = input("Insérez le code ISBN : ")
        article = OpenLibraryAPIResearch(APIResearch(OPENLIBRARY_ENDPOINT)).get("isbn", isbn)
        book = article["docs"][0]
        print(f'Livre trouvé : {book["title"]}, {book["author_name"][0]}, {book["first_publish_year"]}')
        data = [
            book["title"],
            book["author_name"][0],
            isbn,
            book["first_publish_year"],
            input("Langage (fr | en | nl) : "),
            input("Insérez le code-barre : "),
            get_price()
        ]
        bd = BD(*data)
        print(bd)
        confirmation = input(f"Êtes-vous sûr de vouloir importer l'article {bd.title}? (O/N) ").upper()
        match confirmation:
            case "O":
                Article.articles["bd"].append(bd)
                print("\n\nBD correctement ajoutée.✅\n\n")
                databasestorage.sauvegarder_article()
            case "N":
...
def get_price() -> float:
    while True:
        ...
        try:
            ...
            return prix  # Sortir de la boucle si la conversion réussit
        except ValueError:
            print("\u26A0️ Le prix doit être un nombre entier ou décimal. \u26A0️")
